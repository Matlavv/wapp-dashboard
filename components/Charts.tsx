...
const CustomTooltip = ({ active, payload, label }: CustomTooltipProps) => {
    if (active && payload && payload.length) {
        ...
        if (dateLabel && !dateLabel.includes(':')) {
            dateLabel = new Date(dateLabel).toLocaleDateString('fr-FR', {
                weekday: 'long',
                day: 'numeric',
                month: 'long',
            });
    ...
    return (
        <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={data} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                ...
                <YAxis hide={true} domain={['dataMin - 1', 'dataMax + 5']} />
...
const DAUTooltip = ({ active, payload, label }: CustomTooltipProps) => {
    if (active && payload && payload.length) {
        const dateLabel =
...
export const DAUBarChart = ({ data }: { data: GrowthDataPoint[] }) => {
    const { theme } = useTheme();
            ...
            <BarChart data={data} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                <defs>
                    <linearGradient id="colorDAU" x1="0" y1="0" x2="0" y2="1">
                        ...
                        <stop offset="95%" stopColor="var(--foreground)" stopOpacity={0.3} />
                    </linearGradient>
                </defs>
                <CartesianGrid
                    strokeDasharray="3 3"
